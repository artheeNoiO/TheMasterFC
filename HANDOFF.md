...
- ✅ ซ่อนปุ่มเร่งสปีด/ข้ามแมตช์ในโหมดออนไลน์จริง (`fullOnlineMode` ถูกส่งเข้า `LiveMatchModal` แล้ว) — `293f3ba`
- ✅ หน้าจอเลือกโหมด Sandbox/Online หลังสมัคร/ตั้งโปรไฟล์ครั้งแรก (`ModeSelectScreen`) — `eda41ef`
- ✅ แก้บั๊กเซฟข้ามบัญชีรั่ว (`migrateLegacySavesToUser` ลบทิ้งแล้ว) + เซ็น token ฝั่ง Cloudflare Pages ด้วย HMAC จริง (`auth-cf.js`) แทน `game:<userId>` plaintext — `d1d693f`
- ✅ แก้บั๊กใหญ่ชาร์ดไม่แชร์กัน (`createClubForUser` เดิมสร้างชาร์ดบอทส่วนตัวทุกคน) — ตอนนี้ผู้เล่นจริงเข้าไปแทนที่บอทในชาร์ดเดียวกัน ยืนยันด้วย `Start-Test-Server.bat` (2 ผู้เล่นจริงตกชาร์ดเดียวกัน) — `162e266`
- ✅ RadarChartSVG + PlayerDetailModal (คลิก 📊 ข้างชื่อนักเตะใน Squad/Tactics) — `162e266`
- ✅ ขัดภาพแมตช์สดเพิ่ม (ease timeScale, เขี่ยกลางสนามจริงหลังประตู, วิ่งเร่ง/ผ่อนความเร็วแทนวาปทิศ) — `40dba68`

**ยังไม่ได้ทำ:**
5. **โบนัส "ดูสด"** — ยังไม่ได้ตกลงสูตรที่แน่นอน แค่ concept "เปลี่ยนตัว/ปรับแทคติกเองระหว่างเกม = ได้เปรียบกว่าออโต้" ต้องคุยกับ user เรื่องตัวเลขจริงก่อนใส่โค้ด (เช่น โบนัสมูดนักเตะ % หรือแม่นยำในการยิง)

**D. งานค้างเก่าก่อนหน้า (ยังไม่ได้แตะ):**
...
- **ระบบออนไลน์:** auth token เซ็นด้วย `server/src/lib/auth-token.js` (Render) และ `client/functions/lib/auth-cf.js` (Cloudflare Pages Function, ใช้ secret เดียวกัน) · shard/day-tick อยู่ที่ `server/src/services/gameService.js` (`runDayTickForShard`, `prepareUserLiveMatch`/`finishUserLiveMatch` = ล็อกสกอร์จริงตอน kickoff กัน client ปลอมสกอร์, `createClubForUser` แชร์ชาร์ดจริงแล้วไม่แยกบอทต่อคน) · เวลา 9-20น. อยู่ที่ `game-version.js` (`isMatchWindowOpen`/`isMarketWindowOpen`) · เสนอซื้อนักเตะตรง: server `negotiationService.js`+`routes/negotiations.js` (`/api/negotiations/*`) ↔ client `client/src/lib/online-negotiations.js` + `OnlineMarketView` ใน `football-manager.jsx` — **เชื่อมครบแล้วทั้ง 2 ฝั่ง** · `liveMatchService.js` (แมตช์อัตโนมัติตามเวลาจริงผ่าน `Match.status/kickoffAt/eventsJson`) เขียนเสร็จแต่ **ยังไม่ได้ wire เข้า route ไหนเลย** (ดู "ทำต่อ" หัวข้อ B)
...
- [2026-07-05 เช็คสถานะ] เซสชันนี้เปิดมาเช็คความคืบหน้า (`ทำถึงไหนแล้ว`) — พบว่า HANDOFF.md ค้างมาตั้งแต่ commit `d95ee75` (4 ก.ค.) ทั้งที่มี commit ใหม่ต่อมาอีก 6 ตัวถึงวันนี้ (`293f3ba`→`bc4850d`) ที่ไม่เคยบันทึกไว้เลย: เชื่อม negotiations client เข้า UI จริงแล้ว (`293f3ba`), เพิ่มหน้าเลือกโหมด Sandbox/Online (`eda41ef`), แก้บั๊กเซฟข้ามบัญชีรั่ว+เซ็น token Cloudflare ด้วย HMAC (`d1d693f`), ขัดภาพแมตช์สด (`40dba68`), แก้บั๊กชาร์ดไม่แชร์กันจริง+เพิ่ม RadarChart+เขียน `liveMatchService.js` ที่ยังไม่ได้ wire ใช้งาน (`162e266`), เพิ่ม `render.yaml` ย้าย server ไป Render+Postgres (`bc4850d`) — อัปเดตไฟล์นี้ให้ตรงกับสถานะจริงทั้งหมด (สถานะด่วน/ทำต่อ/ไฟล์สำคัญ) ก่อนทำงานต่อ ยังไม่ได้แก้โค้ดอะไรในเซสชันนี้
